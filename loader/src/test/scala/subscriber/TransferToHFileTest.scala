...
class TransferToHFileTest  extends FlatSpec with BeforeAndAfterAll with Matchers {
  ...
  private val testLabelName = "friends_rel"
  ...
  override def beforeAll(): Unit = {
    ...
    // 2. create label
    if(Management.findLabel(testLabelName).isEmpty) {
  ...
  "buildDegrees" should "build degrees on edge format data without direction" in {
    ...
    degrees should contain(DegreeKey("a", testLabelName, "out") -> 3L)
    degrees should contain(DegreeKey("b", testLabelName, "out") -> 2L)
    ...
    degreesAutoCreated should contain(DegreeKey("a", testLabelName, "out") -> 3L)
    degreesAutoCreated should contain(DegreeKey("b", testLabelName, "out") -> 2L)
    degreesAutoCreated should contain(DegreeKey("b", testLabelName, "in") -> 1L)
    degreesAutoCreated should contain(DegreeKey("c", testLabelName, "in") -> 1L)
    degreesAutoCreated should contain(DegreeKey("d", testLabelName, "in") -> 2L)
    degreesAutoCreated should contain(DegreeKey("e", testLabelName, "in") -> 1L)
  }
  ...
  "buildDegrees" should "build degrees on edge format data with direction" in {
    ...
    degrees should contain(DegreeKey("a", testLabelName, "out") -> 3L)
    degrees should contain(DegreeKey("b", testLabelName, "out") -> 2L)
    degrees should contain(DegreeKey("b", testLabelName, "in") -> 1L)
    degrees should contain(DegreeKey("c", testLabelName, "in") -> 1L)
    degrees should contain(DegreeKey("d", testLabelName, "in") -> 2L)
    degrees should contain(DegreeKey("e", testLabelName, "in") -> 1L)
  }
}